...
`app/tools/make_project.py`. Working name Nightjar (Brett hated "Hush"; he
has not signed off on the new name, so keep the rename cheap). The audio
engine in `app/Nightjar/Audio/` synthesises every sound and is the part
Brett wants kept; everything around it was rebuilt 2026-09-05. Read
`app/README.md` before touching it. No Swift toolchain exists in the web
sessions: run `python3 app/tools/check_swift.py` and
`app/tools/verify_project.py`, and treat every SwiftUI change as unverified
until Brett builds it. `.github/workflows/testflight.yml` ships it to
TestFlight from `main` once Brett adds the six secrets the README lists.
The site pages under `/nightjar/` are the app's support, privacy and terms
URLs and must stay live once the app is on TestFlight.
