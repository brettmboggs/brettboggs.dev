...
## Nightjar (the iPhone app, in app/)

A native SwiftUI sleep app lives in `app/`, one Xcode target generated by
`app/tools/make_project.py`. It is called **Slumbio** on the App Store
(record `6808987732`); every single word worth having was taken. The bundle
id `dev.brettboggs.nightjar`, the Xcode target, the `app/Nightjar/` folder
and the Application Support directory keep the old name on purpose: they are
invisible, and renaming the last one would orphan the mixes and journal
already on Brett's phone. The audio engine in `app/Nightjar/Audio/`
synthesises every sound and is the part Brett wants kept; everything around
it was rebuilt 2026-09-05. Read `app/README.md` before touching it. No Swift
toolchain exists in the web sessions: run `python3 app/tools/check_swift.py`
and `app/tools/verify_project.py`, and treat every SwiftUI change as
unverified until Brett builds it. `.github/workflows/testflight.yml` ships it
to TestFlight on every push to `main` that touches `app/`; all seven secrets
the README lists are set. The site pages under `/slumbio/` are the app's
