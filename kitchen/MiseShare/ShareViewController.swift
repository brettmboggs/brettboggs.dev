...
/// "Share › Mise" from Safari. Takes the page's address, drops it in the App
...
final class ShareViewController: UIViewController {
    private let groupID = "group.dev.brettboggs.mise"
        ...
        collectURL { [weak self] url in
            DispatchQueue.main.async {
                ...
                if let url, self.store(url) {
                    self.show(message: "Saved to Mise", done: true)
                } else {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if done && message.hasPrefix("Saved") {
                        Text("Open Mise to check it and save it.")
                            .font(.system(size: 13))
