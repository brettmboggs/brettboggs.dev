...
extension UTType {
    /// One recipe, with its photo inside. Opens in Ladle on another phone.
    static let ladleRecipe = UTType(exportedAs: "dev.brettboggs.ladle.recipe", conformingTo: .json)
    /// The whole book: recipes, pantry, list, plan, settings.
    static let ladleLibrary = UTType(exportedAs: "dev.brettboggs.ladle.library", conformingTo: .json)
}
...
struct RecipeFile: Codable {
    var format: String = "ladle-recipe"
    var version: Int = 1
    var recipe: Recipe
    var photo: Data?
    var scans: [Data] = []
    ...
    init(from decoder: Decoder) throws {
        ...
        format = try c.decodeIfPresent(String.self, forKey: .format) ?? "ladle-recipe"
...
struct LibraryFile: Codable {
    var format: String = "ladle-library"
    var version: Int = 1
    ...
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        format = try c.decodeIfPresent(String.self, forKey: .format) ?? "ladle-library"
        version = try c.decodeIfPresent(Int.self, forKey: .version) ?? 1
...
/// A recipe as something the share sheet can hand over as a `.ladle` file.
struct RecipeDocument: Transferable {
    ...
    init(recipe: Recipe) {
        ...
        fileName = Transfer.fileName(recipe.title, ext: "ladle")
    }

    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(exportedContentType: .ladleRecipe) { document in
...
struct LibraryDocument: Transferable {
    ...
    init(library: Library) {
        ...
        fileName = Transfer.fileName("Ladle cookbook \(stamp)", ext: "ladlebook")
    }

    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(exportedContentType: .ladleLibrary) { document in
            let url = FileManager.default.temporaryDirectory.appendingPathComponent(document.fileName)
            try document.data.write(to: url, options: .atomic)
            return SentTransferredFile(url)
        }
...
enum ImportError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unknownFormat: return "That is not a Ladle recipe file."
        }
    }
}

enum FileImporter {
    /// Reads a `.ladle` or `.ladlebook` file into the library.
    @MainActor
    ...
    static func importData(_ data: Data, into library: Library) throws -> ImportOutcome {
        if let file = try? Persistence.decoder.decode(LibraryFile.self, from: data), file.format == "ladle-library" {
            var count = 0
        ...
        if let file = try? Persistence.decoder.decode(RecipeFile.self, from: data), file.format == "ladle-recipe" {
