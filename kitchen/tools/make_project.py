...
def build() -> str:
    ...
    widget_support = [
        Path(f"{WIDGET_NAME}/Info.plist"),
        Path(f"{WIDGET_NAME}/{WIDGET_NAME}.entitlements"),
    ]
    ...
    app_files = app_sources + resources + [Path(f"{APP_NAME}/Info.plist"), Path(f"{APP_NAME}/{APP_NAME}.entitlements")]
    ...
    # The share extension shares one App Group with the app: that is the
    # inbox the links land in. Extension-safe API only, so nothing in it can
    # reach UIApplication.
    widget_common = {
