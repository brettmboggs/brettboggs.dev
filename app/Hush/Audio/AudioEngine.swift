...
final class AudioEngine {
    ...
    private func activateSession() {
        do {
            ...
            NSLog("Hush: audio session activation failed: \(error.localizedDescription)")
        }
    }
    ...
    private func buildGraph() {
        ...
        guard let format = AVAudioFormat(standardFormatWithSampleRate: rate, channels: 2) else {
            NSLog("Hush: could not create a render format at \(rate) Hz")
            return
        }
    ...
    func start() -> Bool {
        ...
        do {
            ...
            NSLog("Hush: engine start failed: \(error.localizedDescription)")
            ...
            do {
                ...
                NSLog("Hush: engine restart failed: \(error.localizedDescription)")
                isRunning = false
                return false
            }
        }
