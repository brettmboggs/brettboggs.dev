...
final class RecordingStream {
    ...
    init?(resource: String, outputFormat: AVAudioFormat, ring: AudioRingBuffer) {
        ...
        guard let url = Bundle.main.url(forResource: resource, withExtension: "m4a") else {
            NSLog("Hush: recording '\(resource).m4a' is not in the bundle")
            return nil
        }
        do {
            ...
            NSLog("Hush: could not open '\(resource).m4a': \(error.localizedDescription)")
            return nil
        }
        ...
        guard totalFrames > 4096 else {
            NSLog("Hush: '\(resource).m4a' is too short to loop")
            return nil
        }
        ...
        guard
            ...
            NSLog("Hush: could not set up conversion for '\(resource).m4a'")
            return nil
        }
        converter = conv
        inputBuffer = input
    ...
    func refill() {
        while ring.availableToWrite >= Int(outputChunk) {
            ...
            if status == .error {
                NSLog("Hush: conversion failed: \(error?.localizedDescription ?? "unknown")")
                return
            }
    ...
    private func readBody(frames: AVAudioFrameCount) -> Int {
        do {
            ...
            NSLog("Hush: read failed: \(error.localizedDescription)")
            return 0
        }
    }
...
final class RecordingStreamPool {
    ...
    private let queue = DispatchQueue(
        label: "dev.brettboggs.hush.recordings",
        qos: .userInitiated
    )
