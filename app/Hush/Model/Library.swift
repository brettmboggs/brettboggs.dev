...
final class Library {
    ...
    static func load() -> Library {
        ...
        library.publishToWidgets()
        return library
    }

    /// Presets first, then the user's own, newest first.
    var allMixes: [Mix] {
        Mix.presets + userMixes.sorted { $0.createdAt > $1.createdAt }
    }

    func mix(withID id: UUID) -> Mix? {
        allMixes.first { $0.id == id }
    }
    ...
    private func publishToWidgets() {
        SharedStore.writeMixes(allMixes)
    }
}
