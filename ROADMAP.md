...
| 8  | Embedded 3D/CAD viewer           | LIVE    | /lab/parts/: orbit viewer on the site's own three (model-viewer conflicted with three 0.185), warm studio light, soft shadow, auto-rotate off under reduced motion. Pipeline: tools/parts/convert.py (Blender headless, STL to GLB, mm to meters, decimate, stacks assemblies) driven by tools/parts/parts.json. First three: Ridge AirTag mod, SM7B spacer, threaded tube + cap — Brett's best-guess originals, swap if any is not his. Adding a part = add to parts.json, run converter, add a line to parts.astro. |
